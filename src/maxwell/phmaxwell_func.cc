...
PHMaxwellFunc<dim> PHMaxwellFunc<dim>::create_from_parameters(ParameterHandler &prm) {
    auto E_func = std::make_shared<Functions::ParsedFunction<dim>>(3);
    auto B_func = std::make_shared<Functions::ParsedFunction<dim>>(3);
    auto phi_func = std::make_shared<Functions::ParsedFunction<dim>>(1);
    auto psi_func = std::make_shared<Functions::ParsedFunction<dim>>(1);
