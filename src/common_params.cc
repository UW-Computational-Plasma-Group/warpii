...
R"(The number of non-symmetry dimensions in the problem.
...
R"(The number of distinct non-periodic boundary_ids in the domain. This must match
the state of the actual mesh as constructed from a file or by populating a triangulation.)");
    }
    ...
    void declare_n_writeout_frames(ParameterHandler& prm) {
        prm.declare_entry("n_writeout_frames", "10", Patterns::Integer(0),
                "The number of frames, or snapshots, of the solution to write out. Frames are evenly spaced in time, and frame 0 is the initial condition. Setting this to 10 will write out frame 0 through frame 10, for a total of 11 snapshots.");
    }
}
